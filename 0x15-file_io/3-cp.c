...
void error_handler(int exit_code, char *message, char type, ...)
{
	...
	va_start(arg, type);
	if (type == 's')
		dprintf(STDERR_FILENO, message, va_arg(arg, char *));
	else if (type == 'd')
		dprintf(STDERR_FILENO, message, va_arg(arg, int));
	else if (type == 'N')
...
int main(int argc, char *argv[])
{
	...
	while ((byte_read = read(fd_s, buffer, 1024)) > 0)
	{
		byte_written = write(fd_d, buffer, byte_read);
		if (byte_written == -1)
			error_handler(99, "Error: Can't write to %s\n", 's', argv[2]);
	}
